...
public class OrderController {
  ...
  public String paymentPage(Model model, Authentication authentication) {
    ...
    List<Invoice> InvoiceListFromDB =
        invoiceList.stream().filter(invoice -> !invoice.isPaid()).collect(Collectors.toList());
    List<Invoice> PaidInvoiceListFromDB =
        invoiceList.stream().filter(Invoice::isPaid).collect(Collectors.toList());
